...
bool estunecasevoisine(GameState*map, int i, int k, Color car){//fonction qui retourne true si la case de coordonnée i,k possède un voisin de valeur car
    ...
    else if (i==size-1 && k==size-1){//coin bas droite
        return(map ->map[(i-1)*size+k]==car||map ->map[(i)*size+k-1]==car);
    }
...
int main(int argc, char** argv) {
    ...
    for (int partie = 1; partie <= 500; partie++) {
        ...
        while (verifie(&game) == false) {
            ...
            Color couleur_choisie = aleatoire();
            miseajour_map(&game, (Color)1, couleur_choisie);
            if (verifie(&game)) {
                aleatoire_victoires++;
                break;
            }
            Color couleur_choisie2 = glouton(&game, (Color)2);
            miseajour_map(&game, (Color)2, couleur_choisie2);
            if (verifie(&game)) {
                glouton_victoires++;
                break;
            }
    ...
    printf("IA Aléatoire a gagné %d parties.\n", aleatoire_victoires);
